...
var calculateStats = function calculateStats(data) {
  ...
  var game = new Game();
  store.gamesOver.forEach(function (element) {
    game.cells = element.cells;
    var winningToken = game.winningToken();
...
var refreshStats = function refreshStats() {
  ...
  api.getStatsOverFalse().then(function (data) {
...
var onSignIn = function onSignIn(event) {
  ...
  api.signIn(data).then(ui.onSignInSuccess).then(function () {
    gameApi.createGame().then(function (data) {
      store.game = data.game;
    });
  }).then(refreshStats).catch(ui.onSignInFailure);
};
...
var getStatsOverTrue = function getStatsOverTrue() {
  return $.ajax({
...
var getStatsOverFalse = function getStatsOverFalse() {
  return $.ajax({
...
module.exports = {
  ...
  signOut: signOut,
  getStats: getStats,
  getStatsOverTrue: getStatsOverTrue,
  getStatsOverFalse: getStatsOverFalse
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var isNotEmpty = function isNotEmpty(currentValue) {
  return currentValue !== '';
};
...
Game.prototype.winCheck = function () {
  ...
  for (var _i = 0; _i < 3; _i++) {
    ...
    // // checks for undefined
    // console.log('isNotEmpty:', isNotEmpty(this.cells[i]))
    if (isNotEmpty(this.cells[_i])) {
      if (this.verticalCheck(_i)) {
        // change current game condition
        this.over = true;
        return true;
      }
...
Game.prototype.winningToken = function () {
  ...
  for (var _i2 = 0; _i2 < 3; _i2++) {
    ...
    // // checks for undefined
    // console.log('isNotEmpty:', isNotEmpty(this.cells[i]))
    if (isNotEmpty(this.cells[_i2])) {
      if (this.verticalCheck(_i2)) {
        // change current game condition
        return this.cells[_i2];
      }
    }
  }
  // diagonal from top left to bottom right check
...
var resumeGame = function resumeGame(board, gameID) {
...
var placeTokenInCell = function placeTokenInCell(currentCell, playerToken) {
  currentCell.children('.cell-content').text(playerToken);
};

var boardUiUpdateFail = function boardUiUpdateFail(error) {
  console.log(error);
  $('#game-alert').removeClass().addClass('game-message text-danger').text('Uh-oh! Problem updating game. Please try again');
};

module.exports = {
  toggleCurrentPlayerAlert: toggleCurrentPlayerAlert,
  showInvalidMoveWarning: showInvalidMoveWarning,
  showWinMessage: showWinMessage,
  showStalemateMessage: showStalemateMessage,
  resetBoardUi: resetBoardUi,
  placeTokenInCell: placeTokenInCell,
  resumeGame: resumeGame,
  boardUiUpdateFail: boardUiUpdateFail
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var updateState = function updateState(board, index) {
  // debugger
  return $.ajax({
...
var gameEvents = __webpack_require__(17);
...
var boardUi = __webpack_require__(8);
...
var userApi = __webpack_require__(5);
...
var onCellClick = function onCellClick(event) {
  ...
  if (!board.over && !board.collisionCheck(cellVal)) {
    if (!store.user) {
      ...
      board.play(cellVal);
      api.updateState(board, cellVal).then(function (data) {
        store.game = data.game;
        // console.log(store.game)
      }).then(ui.placeTokenInCell(currentCell, board.currentPlayer.toUpperCase())).catch(ui.boardUiUpdateFail);
    }
    board.switchToken();
...
var onClearBoard = function onClearBoard() {
  ...
  if (store.user) {
    ...
    api.createGame().then(function (data) {
      store.game = data.game;
    }).then(ui.resetBoardUi).catch(function (error) {
      console.log(error);
    });
  } else {
    ui.resetBoardUi();
...
var onResumePreviousGame = function onResumePreviousGame() {
  ...
  userApi.getStatsOverFalse().then(function (data) {
    store.gamesOpen = data.games;
    ...
    if (store.gamesOpen && store.gamesOpen.length !== 0) {
      ...
      if (store.gamesOpen.length >= 2) {
        ...
        var _xCount = 0;
        var _oCount = 0;
        for (var _i = 0; _i < board.cells.length; _i++) {
          if (board.cells[_i] === 'x') {
            _xCount++;
          } else if (board.cells[_i] === 'o') {
            _oCount++;
          }
        }
        if (_xCount > _oCount) {
          board.currentPlayer = 'o';
        } else if (_oCount >= _xCount) {
          board.currentPlayer = 'x';
        }
